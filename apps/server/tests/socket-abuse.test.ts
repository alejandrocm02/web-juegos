 ...
 *
 * No comprueban reglas de juego, sino que el servidor aguanta a un cliente
 * hostil: rafagas de mensajes, payloads enormes y eventos sin datos que antes
 * se saltaban el limitador.
 */
...
describe('resistencia de la capa de sockets', () => {
  it('rechaza una rafaga de eventos sin payload que antes esquivaba el limitador', async () => {
    const socket = connect();
    socket.emit(CLIENT_EVENTS.createRoom, { name: 'Rafaga' });
    await once<RoomSummary>(socket, SERVER_EVENTS.roomState);

    const limited = once<AppError>(
      socket,
      SERVER_EVENTS.error,
      (error) => error.code === 'RATE_LIMITED',
    );
    // `room:start` no lleva datos: el limitador es la unica defensa.
    for (let i = 0; i < env.SOCKET_RATE_LIMIT + 20; i += 1) {
  ...
  it('rechaza crear salas cuando el proceso esta al completo', () => {
    const created: string[] = [];
    try {
      while (!manager.isAtCapacity && created.length < env.MAX_ROOMS + 1) {
        created.push(manager.create().code);
      }
      expect(manager.isAtCapacity).toBe(true);
      expect(() => manager.create()).toThrow(/Limite de salas/);
    } finally {
      for (const code of created) manager.remove(code);
    }
  });
});
