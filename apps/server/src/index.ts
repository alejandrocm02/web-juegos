...
export async function createApp() {
  ...
  // El cliente compilado no carga nada de terceros: ni fuentes, ni CDN, ni
  // analitica. Eso permite una politica estricta en produccion. En desarrollo
  // se desactiva porque Vite sirve el cliente desde otro origen con HMR.
  app.use(
    helmet({
      contentSecurityPolicy: env.isProduction
        ? {
            directives: {
              defaultSrc: ["'self'"],
              scriptSrc: ["'self'"],
              // Tailwind y las animaciones inyectan estilos en linea.
              styleSrc: ["'self'", "'unsafe-inline'"],
              imgSrc: ["'self'", 'data:', 'blob:'],
              fontSrc: ["'self'", 'data:'],
              // Socket.IO negocia por HTTP y asciende a WebSocket.
